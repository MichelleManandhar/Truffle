...
class RetailController extends Controller {
    ...
    {
        if (isset($_GET['rid']) && $_GET['rid'] != null && $_GET['rid'] > 0) {
            Yii::$app->session['discount'] = false;
            Yii::$app->session['master-password'] = false;
            
            $retail_id = (int) $_GET['rid'];
            $retail = Query::queryOne("SELECT * FROM retail WHERE id = " . $retail_id);
            ...
            if ($retail != null) {
                ...
                $kot = Kot::find()->WHERE(['retail_id' => $retail_id])
                                    ->andWhere(['<>','status', 1])
    ...
    public function actionList()
    {
        if(Yii::$app->request->isAjax){
            $retail = Query::queryAll("SELECT * FROM retail WHERE status != 1 AND is_confirmed = " . Yii::$app->params['retail_confirmation']['done_nothing']." ORDER BY created_on DESC");
            echo json_encode(array('status' => 'true' , 'retail' => $retail)); die;
        }
    ...
    public function actionCancel()
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != ''){
    ...
    public function actionConfirm()
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != ''){
                $retail = Retail::find()->WHERE(['id' => $_POST['retail_id']])->one();
                $retail->is_confirmed = Yii::$app->params['retail_confirmation']['confirm'];
                if($retail->update()){
                    echo json_encode(true); die;
                }
            }
    ...
    {   
        ...
        if($latest != '' &&  date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != ''){
                $retail = Retail::find()->WHERE(['id' => $_POST['retail_id']])->one();
                $retail->is_confirmed = Yii::$app->params['retail_confirmation']['cancel'];
                if($retail->update()){
                    echo json_encode(true); die;
                }
            }
    ...
    public function actionTempOrder() 
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['item'] != '' && $_POST['retail_id'] != ''){
                $item_id = Query::queryOne("SELECT id FROM `menu_item` WHERE `slug` = '" . $_POST['item'] . "'");
                $temp_item = TempItems::find()->WHERE(['item_id' => $item_id , 'retail_id' => $_POST['retail_id']])->one();
                if($temp_item != null){
                    ...
                    $temp_item->retail_id= $_POST['retail_id'];
    ...
    public function actionUpdateTempOrderQuantity()
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && isset($_POST['item']) && $_POST['item'] != '' && isset($_POST['retail_id']) && $_POST['retail_id'] != ''){
                $item_id = Query::queryOne("SELECT id FROM `menu_item` WHERE `slug` = '" . $_POST['item'] . "'");
                $temp_order = TempItems::find()->WHERE(['item_id' => $item_id , 'retail_id' => $_POST['retail_id']])->one();
                if($temp_order != null){
    ...
    public function actionTempOrderList() 
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && isset($_POST['item']) && $_POST['item'] != '' && isset($_POST['retail_id']) && $_POST['retail_id'] != '') {
                ...
                $temp_order_list = TempOrderList::find()->WHERE(['item_id' => $item_id , 'retail_id' => $_POST['retail_id'], 'type' => Yii::$app->params['item_type']['regular']])->one();
                ...
                if($temp_order_list != null) {
                    ...
                    $temp_order_list->retail_id = $_POST['retail_id'];
    ...
    public function actionSpecialTempOrderList() 
    {
        if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != '') {
            ...
            foreach($_POST['item'] as $item):
                $batch[] = [$_POST['retail_id'], $item['id'], $item['quantity'], $item['price'], Yii::$app->params['item_type']['special']];
            endforeach;

            Query::batchInsert('temp_order_list', array('retail_id', 'item_id', 'quantity', 'price', 'type'), $batch);
    ...
    public function actionDeleteTempOrderList() 
    {
        ...
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != null){
                Query::execute("DELETE FROM temp_order_list WHERE retail_id = " . $_POST['retail_id']);
                ...
                $kot['retail_id'] = $kot_list['retail_id'];
...
// Change HEre
    public function actionUpdateRetail() 
    {
        $latest = Misc::getLatestDate();
        if( $latest != '' && date('Y-m-d') < $latest['latest_date'] ) { 
            echo json_encode(false); die;
            ...
            if(Yii::$app->request->isAjax && isset($_POST) && $_POST['retail_id'] != null){
                $retail = Retail::find()->WHERE(['id' => $_POST['retail_id']])->one();
                $retail->$_POST['field'] = $_POST['value'];
                if($retail->update()){
                    if($_POST['field']=='customer_phone'){
                        if($retail->customer_phone!=''){
                            $customer = Query::queryOne("SELECT * FROM customer WHERE phone = ".$retail->customer_phone."");
                            if($customer){
