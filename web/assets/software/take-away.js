...
/*$(document).on('submit', '.take-away-form', function (e) {
    e.preventDefault();
    $.ajax({
        type: 'POST',
        url : $('.take-away-form').attr('action'),
        data: $('.take-away-form').serialize(),
        async: false,
        success: function (response) {
            ...
            if(res.status == 'logout') {
                ...
                if(res != false){
                    window.location.href = baseUrl + '/' + take_away_version + '/?tid='+res.take_away_id;
                }
            }
        }
    });
...
$(document).one('click', '.btn-take-away', function (e) {
    e.preventDefault();
    $.ajax({
        type: 'POST',
        url : baseUrl + '/table/take-away-now',
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res.status == 'logout') {
    ...
    $('.btn-take-away').prop('disabled', true);
});

$(document).on('click' , '.close-take-away-modal-btn' , function (e) {
    e.preventDefault();
    $('.form-group').each(function () { $(this).removeClass('has-error'); });
    $('.help-block').each(function () { $(this).remove(); });
    $('.take-away-now').find('.take-away-form')[0].reset();
    $('.take-away-now').modal('toggle');
});

$(document).on('click', '.btn-take-away-list', function () {
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/take-away/list',
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
                var take_away_list_content = "";
                var selected_count = 0;
                $.each(res.take_away, function (i,val) {
                    selected_count++;
                    ...
                    take_away_list_content += 
                                            '<tr class="take-away" data-take_away_id="'+val.id+'">'+
                                                ...
                                                '<td>';
                                                if(val.status == 0) {
                                                    take_away_list_content += '<a href="'+ baseUrl +'/' + take_away_version + '/?tid='+val.id+'"><button type="button" class="btn btn-xs btn-info view-take-away">View</button></a>'+
                                                                                '<button type="button" class="btn btn-xs btn-danger cancel-take-away">Cancel Order</button>';
                                                }else {
                                                    take_away_list_content += '<button type="button" class="btn btn-xs btn-info btn-confirm-take-away">Confirm TakeAway</button>'+
                                                                            '<button type="button" class="btn btn-xs btn-danger btn-cancel-ordered-take-away">Cancel TakeAway</button>';
                                                }
                                                    
                    take_away_list_content += '</td>'+
                                            '</tr>';
                });
                $('.take-away-list-content').html(take_away_list_content);
                $('.take-away-list').modal('show');
                $('.take-away-tables-data').dataTable();
            }
        }
    });
});

$(document).on('click', '.btn-confirm-take-away', function () {
    var $this = $(this);
    var take_away_id = $this.parents('.take-away').data('take_away_id');
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/take-away/confirm',
        data :{take_away_id : take_away_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
                $this.parents('.take-away').remove();
                rearrangeDeliverySN();
            }else{
...
$(document).on('click', '.btn-cancel-ordered-take-away', function () {
    var $this = $(this);
    var take_away_id = $this.parents('.take-away').data('take_away_id');
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/take-away/cancel-ordered',
        data :{take_away_id : take_away_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
...
$(document).on('click', '.cancel-take-away', function () {
    var $this = $(this);
    var take_away_id = $this.parents('.take-away').data('take_away_id');
    bootbox.prompt({
...
$(document).on('click', '.select-item', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/take-away/temp-order-list',
        data: {item: item, take_away_id: take_away_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
...
$(document).on('click', '.send-selected-special-item-list', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/take-away/special-temp-order-list',
        data: $form.serialize() + '&take_away_id='+take_away_id,
        async: false,
        success: function (response) {
            var data = $.parseJSON(response);
            if(data != false){
            ...
            if (data != false) {
                ...
                $.each(data.kot_category, function (key , category) {
                    ...
                    if (data.kot_category_count == count) {
                        ...
                        var take_away_id = data.take_away_id;
                        var selected_count = 0;
                        ...
                        $.ajax({
                            type : 'POST',
                            url  : baseUrl + '/take-away/delete-temp-order-list',
                            data : {take_away_id : take_away_id , kot_id : kot_id},
                            async: false,
                            success: function (response) {
                                var res = $.parseJSON(response);
                                if(res != false){
...
$(document).on('change', '.quantity', function () {
    ...
    if ($this.parents('.order-desk').length) {
        ...
        if(list_type == 'all_list'){
             $.ajax({
                type: 'POST',
                url : baseUrl + '/take-away/update-temp-order-quantity',
                data: {item : temp_item , price : price , take_away_id : take_away_id , quantity : quantity},
                async:false,
                success: function (response) {
                    var res = $.parseJSON(response);
                    if(res != false){
            ...
            $.ajax({
                type : 'POST',
                url  : baseUrl + '/take-away/update-temp-order-list-quantity',
...
$(document).on('click', '.remove-item', function (e) {
    ...
    if(temp_order_list_id != null){
        $.ajax({
            type: 'POST',
            url : baseUrl + '/take-away/remove-temp-order-list',
...
$(document).on('blur', '.send-data', function () {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/take-away/update-take-away',
        data: {take_away_id: take_away_id , field : field , value : value},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            $this.val(res.data);
...
function rearrangeTakeAwaySN() {
    $('.take-away').find('.sn').each(function (i, val) {
        $(this).html(i+1);
    });
}
