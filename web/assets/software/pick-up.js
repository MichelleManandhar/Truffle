...
$(document).one('submit', '.pick-up-form', function (e) {
    e.preventDefault();
    $.ajax({
        type: 'POST',
        url : $('.pick-up-form').attr('action'),
        data: $('.pick-up-form').serialize(),
        async: false,
        success: function (response) {
            ...
            if(res.status == 'logout') {
                ...
                if(res != false){
                    window.location.href = baseUrl + '/' + pick_up_version + '/?pid='+res.pick_up_id;
                }else {
    ...
    $('.btn-pick-up').prop('disabled', true);
});

$(document).on('click' , '.close-pick-up-modal-btn' , function (e) {
    e.preventDefault();
    $('.form-group').each(function () { $(this).removeClass('has-error'); });
    $('.help-block').each(function () { $(this).remove(); });
    //$('.modal-pick-up-now').find('.pick-up-form')[0].reset();
    $('.pick-up-form')[0].reset();
    $('.modal-pick-up-now').modal('toggle');
});

$(document).on('click', '.btn-pick-up-list', function () {
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/pick-up/list',
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
                var pick_up_list_content = "";
                var selected_count = 0;
                $.each(res.pick_up, function (i,val) {
                    selected_count++;
                    ...
                    pick_up_list_content += 
                                            '<tr class="pick-up" data-pick_up_id="'+val.id+'">'+
                                                ...
                                                '<td>';
                                                if(val.status == 0) {
                                                    pick_up_list_content += '<a href="'+ baseUrl +'/' + pick_up_version + '/?pid='+val.id+'"><button type="button" class="btn btn-xs btn-info view-pick-up">View</button></a>'+
                                                                                '<button type="button" class="btn btn-xs btn-danger cancel-pick-up">Cancel Order</button>';
                                                }else {
                                                    pick_up_list_content += '<button type="button" class="btn btn-xs btn-info btn-confirm-pick-up">Confirm PickUp</button>'+
                                                                            '<button type="button" class="btn btn-xs btn-danger btn-cancel-ordered-pick-up">Cancel PickUp</button>';
                                                }
                                                    
                    pick_up_list_content += '</td>'+
                                            '</tr>';
                });
                $('.pick-up-list-content').html(pick_up_list_content);
                $('.pick-up-list').modal('show');
                $('.pick-up-tables-data').dataTable();
            }
        }
    });
});

$(document).on('click', '.btn-confirm-pick-up', function () {
    var $this = $(this);
    var pick_up_id = $this.parents('.pick-up').data('pick_up_id');
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/pick-up/confirm',
        data :{pick_up_id : pick_up_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
                $this.parents('.pick-up').remove();
                rearrangePickUpSN();
            }else{
...
$(document).on('click', '.btn-cancel-ordered-pick-up', function () {
    var $this = $(this);
    var pick_up_id = $this.parents('.pick-up').data('pick_up_id');
    $.ajax({
        type : 'POST',
        url  : baseUrl + '/pick-up/cancel-ordered',
        data :{pick_up_id : pick_up_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
...
$(document).on('click', '.cancel-pick-up', function () {
    var $this = $(this);
    var pick_up_id = $this.parents('.pick-up').data('pick_up_id');
    bootbox.prompt({
...
$(document).on('click', '.select-item', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/pick-up/temp-order-list',
        data: {item: item, pick_up_id: pick_up_id},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            if(res != false){
...
$(document).on('click', '.send-selected-special-item-list', function (e) {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/pick-up/special-temp-order-list',
        data: $form.serialize() + '&pick_up_id='+pick_up_id,
        async: false,
        success: function (response) {
            var data = $.parseJSON(response);
            if(data != false){
            ...
            if (data != false) {
                ...
                $.each(data.kot_category, function (key , category) {
                    ...
                    if (data.kot_category_count == count) {
                        ...
                        var pick_up_id = data.pick_up_id;
                        var selected_count = 0;
                        ...
                        $.ajax({
                            type : 'POST',
                            url  : baseUrl + '/pick-up/delete-temp-order-list',
                            data : {pick_up_id : pick_up_id , kot_id : kot_id},
                            async: false,
                            success: function (response) {
                                var res = $.parseJSON(response);
                                if(res != false){
...
$(document).on('change', '.quantity', function () {
    ...
    if ($this.parents('.order-desk').length) {
        ...
        if(list_type == 'all_list'){
             $.ajax({
                type: 'POST',
                url : baseUrl + '/pick-up/update-temp-order-quantity',
                data: {item : temp_item , price : price , pick_up_id : pick_up_id , quantity : quantity},
                async:false,
                success: function (response) {
                    var res = $.parseJSON(response);
                    if(res != false){
            ...
            $.ajax({
                type : 'POST',
                url  : baseUrl + '/pick-up/update-temp-order-list-quantity',
...
$(document).on('click', '.remove-item', function (e) {
    ...
    if(temp_order_list_id != null){
        $.ajax({
            type: 'POST',
            url : baseUrl + '/pick-up/remove-temp-order-list',
...
$(document).on('blur', '.send-data', function () {
    ...
    $.ajax({
        type: 'POST',
        url : baseUrl + '/pick-up/update-pick-up',
        data: {pick_up_id: pick_up_id , field : field , value : value},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            $this.val(res.data);
...
$(document).on('change','.pickup_customer_phone',function(){
    var $this       = $(this);
    var value       = $this.val();

    $.ajax({
        type: 'POST',
        url : baseUrl + '/pick-up/check-customer',
        data: { value : value},
        async: false,
        success: function (response) {
            var res = $.parseJSON(response);
            ...
            if(res.exists=='true'){
                ...
                function() {
                    $('.pickup_customer_name').val(res.customer['name'])
                    swal.close();
                });
            }else{
                console.log('false');
...
function rearrangePickUpSN() {
    $('.pick-up').find('.sn').each(function (i, val) {
        $(this).html(i+1);
    });
}
